...
3. **Classify** the bug:
   ...
   - **API bug (TypeScript)**: issue in `apps/api/**` → fix directly or delegate to **API Dev** (with TypeScript service context)
   - **API bug (Python)**: issue in `apps/api-<name>/**` → fix directly or delegate to **API Dev** (with Python service context)
   - **Web bug**: issue in `apps/web/**` or `apps/mobile/**` → fix directly or delegate to **Web Dev**
...
- Check the appropriate contract: `contracts/openapi.yaml` for the main API, `contracts/openapi-<service>.yaml` for additional services
- Check the API implementation in the correct service directory:
  - TypeScript: `apps/api/` (routes, controllers, use-cases, repos)
  - Python: `apps/api-<name>/` (blueprints, controllers, use_cases, repos)
- Check the Web/Mobile implementation (features, api client, pages)
- Look for mismatches between layers or between service contracts
...
## Rules
- Never change any `contracts/openapi*.yaml` without explicit user approval — report contract issues and ask.
- Respect architectural boundaries:
  - API (TypeScript): Clean Architecture + DDD + DI (see [API rules](.github/instructions/api.instructions.md))
  - API (Python): Clean Architecture + DDD + DI (see [Python API rules](.github/instructions/python-api.instructions.md))
  - Web/Mobile: Feature-Sliced (see [Web rules](.github/instructions/web.instructions.md))
- Keep fixes minimal — do not refactor unrelated code.
- If the bug reveals a missing endpoint/field in any contract, report it as: `CONTRACT GAP: <description>`.
